...
func Upload(field string, r *http.Request, maps map[string]interface{}) (*UploadFile, error) {
	...
	uploadType1 := maps["upload_type"]
	uploadType := ""
	if uploadType1 != nil {
		uploadType = uploadType1.(string)
	}
	//数据填充
	_, err = UploadFile.SetUploadFileData(uploadType, file, header)
	if err != nil {
		return nil, err
	}
	//审核 检测 大小，文件后缀
...
//文件基本数据填充
//@upload_type 上传配置
func (c *UploadFile) SetUploadFileData(uploadType string, file multipart.File, header *multipart.FileHeader) (bool, error) {
	var spe string
	...
	//配置
	if uploadType == "" {
		uploadType = "upload_default"
	}
	//配置检测
	if _, err := c.SetConfig(uploadType); err != nil {
		return false, err
	}
	rootPath := ""
	rootPath = c.Config["root_path"]
	if rootPath == "" {
	...
	if strings.Contains(h, "#upload") {
		h = ""
	}
	c.Http = h + strings.Replace(c.Url, "/", "", 1)
	fmt.Println("文件数据：", c)
...
//本地保存
func (c *UploadFile) LocalSaveFile(file multipart.File, path, fileName string) (bool, error) {
	//当前的目录
	dir, _ := os.Getwd()
...
func (c *UploadFile) LocalTmpFileRemove() error {
	rootPathTmp := c.GetLocalTmpPath()
	//当前的目录
	dir, _ := os.Getwd()
	fmt.Println("当前的目录", dir)
	file := dir + rootPathTmp + c.Name
...
func (c *UploadFile) GetLocalTmpPath() string {
	rootPathTmp := c.Config["root_path_tmp"]
	if rootPathTmp == "" {
		rootPathTmp = "/uploads/tmp"
	}
	return rootPathTmp
}
